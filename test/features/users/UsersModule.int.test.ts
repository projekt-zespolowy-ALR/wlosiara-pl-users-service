...
describe("UsersModule", () => {
	...
	describe("v1", () => {
		describe("Empty database", () => {
			...
			test("GET /users/:id with invalid UUID4", async () => {
				const response = await app.inject({
					method: "GET",
					url: "/v1/users/1",
				});
			...
			test("GET /users/:id with valid UUID4", async () => {
				const response = await app.inject({
					method: "GET",
					url: "/v1/users/e8a7b311-367b-4105-a75d-929b930faafa",
				});
				expect(response.statusCode).toBe(404);
			});
		});

		describe("Database with one user", () => {
			test("GET /users", async () => {
				const addUserRequestBody = {
					username: "test2",
					avatarUrl: "test2",
